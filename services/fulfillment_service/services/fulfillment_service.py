...
#         try:
#             gemini_api_key = "AIzaSyD6LeLiU-Oo1CHlrXDG_4CqbOEbAxGu8oM"
#             if gemini_api_key:
...
#         """Parse and validate AI optimization response - FIXED VERSION"""
#         try:
#             # DEBUG: Log the raw response from Gemini
#             logger.info(f"DEBUG: Raw Gemini response: {response[:500]}...")
...
#                 # No JSON found, but response exists - treat as working AI
#                 logger.warning(f"No JSON found in Gemini response, but response exists. Using as text response.")
#                 return {
#                     "ai_recommendations": {"text_response": response},
#                     "primary_quantity": None,
#                     "additional_products": [],
#                     "optimization_strategy": response[:200] + "..." if len(response) > 200 else response,
#                     "ai_confidence": "high",  # ← FIX: Mark as high since Gemini responded
#                     "generated_at": datetime.utcnow().isoformat(),
#                     "parsing_note": "Gemini responded but not in expected JSON format"
#                 }
...
#                 "ai_confidence": "high",  # ← This should work when JSON parsing succeeds
#                 "generated_at": datetime.utcnow().isoformat()
#             }
            
#         except json.JSONDecodeError as e:
#             logger.warning(f"JSON decode failed: {e}")
#             logger.warning(f"Attempted to parse: {json_str if 'json_str' in locals() else 'No JSON extracted'}")
            
#             # IMPROVED: If Gemini responded but JSON parsing failed, still mark as working
#             if response and len(response.strip()) > 10:  # Gemini gave a substantive response
#                 logger.info("Gemini provided response but JSON parsing failed - marking as working AI")
#                 return {
#                     "ai_recommendations": {"raw_response": response},
#                     "primary_quantity": None,
#                     "additional_products": [],
#                     "optimization_strategy": f"AI response (parsing failed): {response[:100]}...",
#                     "ai_confidence": "high",  # ← FIX: Still mark as high if Gemini responded
#                     "generated_at": datetime.utcnow().isoformat(),
...
#             logger.warning(f"Key error in parsing AI response: {e}")
#             # Similar fix for KeyError
#             if response and len(response.strip()) > 10:
#                 return {
#                     "ai_recommendations": {"raw_response": response},
#                     "primary_quantity": None,
#                     "additional_products": [],
#                     "optimization_strategy": f"AI response (key error): {response[:100]}...",
#                     "ai_confidence": "high",  # ← FIX: Mark as high if Gemini responded
#                     "generated_at": datetime.utcnow().isoformat(),
...
#         if not use_ai:
#             return await self.fallback_optimization(shipment_data)

#         return await self.optimize_fulfillment_with_ai(shipment_data)
...
class FulfillmentService:
    ...
    async def optimize_shipment_with_ai(self, shipment_data: Dict, use_ai: bool = True) -> Dict:
        """
        Optimize shipment by evaluating stock levels and AI suggestions.
        """
        try:
            store_id = shipment_data["store_id"]
            product_id = shipment_data["product_id"]
            requested_quantity = shipment_data["requested_quantity"]

            warehouse_item = await self.db.find_one("warehouse_inventory", {"product_id": product_id})
            available = warehouse_item.get("available_stock", 0) if warehouse_item else 0
            primary_quantity = min(requested_quantity, available)

            optimization_result = {
                "product_id": product_id,
                "primary_quantity": primary_quantity,
                "optimization_strategy": "ai_assisted_basic"
            }

            if use_ai:
                recommendations = await self.get_ai_product_recommendations(shipment_data)
                optimization_result["additional_products"] = recommendations.get("recommendations", [])

            return optimization_result

        except Exception as e:
            logger.error(f"Error in optimize_shipment_with_ai: {e}")
            return {
                "optimization_strategy": "fallback",
                "primary_quantity": shipment_data.get("requested_quantity", 0),
                "additional_products": []
            }
    ...
    async def get_ai_product_recommendations(self, request_data: Dict) -> Dict:
        """
        Recommend additional products based on category or related items.
        """
        try:
            product_id = request_data["product_id"]
            requested_quantity = request_data["requested_quantity"]
            ...
            # Fetch other products from same category or related ones
            filter_query = {"product_id": {"$ne": product_id}}

            if related_ids:
                filter_query["product_id"]["$in"] = related_ids
            elif category:
                filter_query["category"] = category

            candidate_products = await self.db.find_many("products", filter_query, limit=5)

            recommendations = []
            for item in candidate_products:
                # Check if available in warehouse
                warehouse_item = await self.db.find_one("warehouse_inventory", {"product_id": item["product_id"]})
                available = warehouse_item.get("available_stock", 0) if warehouse_item else 0

                if available > 0:
                    recommendations.append({
                        "product_id": item["product_id"],
                        "recommended_quantity": min(available, max(1, requested_quantity // 2)),
                        "reasoning": f"Frequently bought with {product_id} or belongs to same category '{category}'"
                    })

            return {"recommendations": recommendations}

        except Exception as e:
            logger.error(f"Error generating AI product recommendations: {e}")
            return {"recommendations": []}
